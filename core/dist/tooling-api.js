...
function getDefaultOrg() {
    try {
        const result = (0, child_process_1.execSync)('sf org display --json', { encoding: 'utf8', stdio: 'pipe' });
...
function getInstanceUrl(orgAlias) {
    const result = (0, child_process_1.execSync)(`sf org display --json -o ${orgAlias}`, { encoding: 'utf8', stdio: 'pipe' });
    const orgInfo = JSON.parse(result);
 ...
 */
async function getMetadataId(orgAlias, metadataType, metadataName) {
    try {
        ...
        if (metadataType === 'Flow') {
            ...
            const result = (0, child_process_1.execSync)(`sfdx force:data:soql:query -q "${query}" -u ${orgAlias} --json --use-tooling-api`, { encoding: 'utf8', stdio: 'pipe' });
            const records = JSON.parse(result).result.records;
            if (!records?.length)
                throw new Error(`No Flow found with label: ${metadataName}`);
            ...
            const records = JSON.parse(result).result.records;
            if (!records?.length)
                throw new Error(`No CustomField found: ${metadataName}`);
            return records[0].Id;
        }
        else {
            ...
            const records = JSON.parse(result).result.records;
            if (!records?.length)
                throw new Error(`No ${metadataType} found: ${searchName}`);
            return records[0].Id;
        }
    }
    catch (error) {
...
//# sourceMappingURL=tooling-api.js.map